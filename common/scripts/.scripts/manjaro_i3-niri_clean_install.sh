...
# 1. Purge WM and X11 Bloat (Common to all minimal Manjaro i3 installs)
...
# 3. Enable TTY1 Autologin (Universal for TTY-based autostart)
...
# 4. Generate .zprofile with conditional Hardware Logic
...
if [[ "\$(hostname)" == "legion" ]]; then
    # NVIDIA/Wayland specific fixes for Legion 5
    ...
    # Common/Generic machine defaults (Intel/AMD)
    export MOZ_ENABLE_WAYLAND=1
    export QT_QPA_PLATFORM="wayland;xcb"
fi
...
# 5. Conditional NVIDIA Modesetting for GRUB
if [[ "$(hostname)" == "legion" ]]; then
    echo "Legion detected: Ensuring NVIDIA modesetting..."
    if ! grep -q "nvidia-drm.modeset=1" /etc/default/grub; then
        sudo sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="/GRUB_CMDLINE_LINUX_DEFAULT="nvidia-drm.modeset=1 /' /etc/default/grub
        sudo update-grub
    fi
else
    echo "Generic machine detected: Skipping NVIDIA GRUB tweaks."
fi

echo "Setup complete for $(hostname). Review ~/.zprofile and reboot."
