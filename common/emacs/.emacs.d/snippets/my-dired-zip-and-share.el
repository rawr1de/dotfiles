...
(defun my-dired-zip-and-share ()
  ...
  (let* ((files (dired-get-marked-files t))
         (zip-name (my-dired--next-share-zip)))
    (if (not files)
      ...
      ;; 2. Refresh the Dired buffer to show the new archive
      (revert-buffer)

      ;; 3. Prompt for the sharing method
      (let ((choice (completing-read "Share archive via: " '("KDE Connect" "Magic Wormhole"))))
        (pcase choice
          ("KDE Connect"
