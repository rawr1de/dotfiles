...
(defun my-kde-send-sms ()
  ...
  (let* ((name (completing-read "Text who? (Choose or type number): "
         ...
         ;; CRITICAL FIX: One final scrub before it hits the KDE CLI
    ...
    (if (string-empty-p phone-id)
      ...
      ;; 7. Auto-close the frame after 1 second
      ;; We pass (selected-frame) at the very end of run-at-time so the lambda remembers it
