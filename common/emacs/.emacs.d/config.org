...
** default terminal → bash (ansi-term)
#+BEGIN_SRC emacs-lisp
...
** open org files as PDF in zathura
#+BEGIN_SRC emacs-lisp
(setq org-file-apps
  '((auto-mode . emacs)
...
;; --- C-w: kill buffer, but prompt if unsaved ---
(defun rdo/close-buffer-or-prompt ()
  ...
  (if (and (buffer-modified-p) (buffer-file-name))
      (when (yes-or-no-p (format "Buffer '%s' is unsaved. Kill anyway? " (buffer-name)))
        (set-buffer-modified-p nil)   ; disarm Emacs's own prompt
