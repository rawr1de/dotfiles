...
[[/home/rdo/Docs/5.systems/1.linux_shit/1.text/emacs/dired-cheat_sheet.pdf][dired-cheat-sheet]]
...
** highlight current line (GUI only)
#+BEGIN_SRC emacs-lisp
...
* gemini_RDO-MODE suggestion
#+BEGIN_SRC emacs-lisp
;; =============================================================================
;; 1. GLOBAL ESCAPE & MINIBUFFER LOGIC
;; =============================================================================

;; Make ESC-quit snappier
(setq last-event-frame nil)

;; Use the built-in "Power Quit" globally (Handles mark, popups, and minibuffer)
(global-set-key (kbd "<escape>") 'keyboard-escape-quit)

;; Universal Hook: Always land in INSERT mode when closing ANY minibuffer
(add-hook 'minibuffer-exit-hook 'xah-fly-insert-mode-activate) 

;; Force XFK to respect the ESC key globally
(with-eval-after-load 'xah-fly-keys
  (define-key xah-fly-key-map (kbd "<escape>") 'keyboard-escape-quit) 
...
;; =============================================================================
;; 2. GLOBAL OVERRIDES (REPLACING RDO-MODE)
;; =============================================================================

;; CAPS LOCK (Home) -> Xah Command Mode (Works in minibuffer too)
...
;; =============================================================================
;; 3. PRIORITY FIXES (FOR MODES THAT STEAL C-w)
;; =============================================================================

(defun rdo-apply-priority-keys ()
  "Force key preferences in high-priority major modes."
  (local-set-key (kbd "C-w") #'rdo/close-buffer-or-prompt))

;; Ensure Dired and Org don't overwrite C-w with their own versions
(add-hook 'dired-mode-hook #'rdo-apply-priority-keys) 
(add-hook 'org-mode-hook #'rdo-apply-priority-keys)
#+END_SRC

** disable ESC modal (XFK) & ESC=keyboard-quit
#+BEGIN_SRC emacs-lisp
;;make ESC-quit snappier (activate if ESC start to behave strangely)
;;(setq last-event-frame nil)
;; 1. Use the built-in "Power Quit" globally
;; This handles: Quitting the mark, closing the minibuffer, and closing pop-up windows.
(global-set-key (kbd "<escape>") 'keyboard-escape-quit)

;; 2. The "Insert Mode" Hook
;; This ensures that whenever you exit the minibuffer (whether by ESC or Enter),
;; you land back in your file ready to type.
(add-hook 'minibuffer-exit-hook 'xah-fly-insert-mode-activate)

;; 3. FORCE xah-fly-keys to respect the Escape key
;; Without this, XFK's internal maps will block the ESC signal.
(with-eval-after-load 'xah-fly-keys
  (define-key xah-fly-key-map (kbd "<escape>") 'keyboard-escape-quit)
  (define-key xah-fly-command-map (kbd "<escape>") 'keyboard-escape-quit)
  (define-key xah-fly-insert-map (kbd "<escape>") 'keyboard-escape-quit))
...
** isearch → copy match to kill-ring  [M-w]
#+BEGIN_SRC emacs-lisp
...
** toggle asterisks around region  [C-8]
#+BEGIN_SRC emacs-lisp
